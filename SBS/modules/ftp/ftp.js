$(document).ready(function(){
        if(window.location.href.indexOf("home.php?m=ftp") != -1 ){
			...
			$(this).contents().find('button, input[type=button], input[type=submit], input[type=reset], .swfuploadbtn').addClass('btn').addClass('btn-sm').addClass('btn-secondary');
			$(this).contents().find('select, input[type=text]').addClass('form-control').removeAttr('style');
